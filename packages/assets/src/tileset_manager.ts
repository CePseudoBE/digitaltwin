...
export abstract class TilesetManager extends AssetsManager implements AsyncUploadable {
     ...
     */
    private async authenticateUser(req: TypedRequest): Promise<number | DataResponse> {
     ...
     */
    async handleGetStatus(req: TypedRequest): Promise<DataResponse> {
        try {
            const { id } = req.params || {}
            if (!id) {
            ...
            const record = asset as DataRecord & Partial<TilesetMetadataRow>
    ...
    override async retrieve(req?: TypedRequest): Promise<DataResponse> {
        try {
            ...
            const response = visibleAssets.map(asset => ({
                ...
                tileset_url: (asset as DataRecord & Partial<TilesetMetadataRow>).tileset_url || '',
                upload_status: (asset as DataRecord & Partial<TilesetMetadataRow>).upload_status || 'completed'
            }))
