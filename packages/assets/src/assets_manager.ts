...
/**
 * A single request in a batch upload. Uses `file` for the base64-encoded data.
 */
...
export abstract class AssetsManager implements Component, Servable, OpenAPIDocumentable {
     ...
     * ```
     */
    private validateBase64(data: unknown): boolean {
     ...
     */
    private extractUploadData(req: TypedRequest): UploadData {
     ...
     */
    private async checkPrivateAssetAccess(asset: DataRecord, req: TypedRequest): Promise<DataResponse | undefined> {
     ...
     */
    async retrieve(req?: TypedRequest): Promise<DataResponse> {
        try {
     ...
     * ```
     */
    async handleUpdate(req: TypedRequest): Promise<DataResponse> {
        try {
            if (!req) {
     ...
     * ```
     */
    async handleGetAsset(req: TypedRequest): Promise<DataResponse> {
        try {
     ...
     * ```
     */
    async handleDownload(req: TypedRequest): Promise<DataResponse> {
        try {
            const result = await this.fetchAssetWithAccessCheck(req)
            if (!result.success) {
                return result.response
     ...
     * ```
     */
    async handleDelete(req: TypedRequest): Promise<DataResponse> {
        try {
            // Authenticate user
            const authResult = await this.authenticateRequest(req)
            if (!authResult.success) {
     ...
     */
    async handleUploadBatch(req: TypedRequest): Promise<DataResponse> {
        try {
            if (!req?.body) {
                return badRequestResponse('Invalid request: missing request body')
            }
     ...
     */
    private validateBatchRequests(requests: BatchUploadItem[]): DataResponse | undefined {
        const config = this.getConfiguration()
     ...
     */
    async handleDeleteBatch(req: TypedRequest): Promise<DataResponse> {
        try {
            if (!req?.body) {
                return badRequestResponse('Invalid request: missing request body')
            }
