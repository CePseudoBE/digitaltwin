    ...
    override async handleUpload(req: TypedRequest): Promise<DataResponse> {
        try {
            ...
            const metadata: MapLayerMetadataRow = {
                ...
                url,
                date: now,
                description: (description as string) || layerInfo.description || 'Map layer',
                source: (body.source as string) || 'uploaded',
    ...
    override async retrieve(): Promise<DataResponse> {
        try {
            ...
            const assetsWithMetadata = assets.map(asset => ({
                ...
                layer_type: (asset as DataRecord & Partial<MapLayerMetadataRow>).layer_type || '',
                layer_name: (asset as DataRecord & Partial<MapLayerMetadataRow>).layer_name || '',
                geometry_type: (asset as DataRecord & Partial<MapLayerMetadataRow>).geometry_type || null,
                properties_count: (asset as DataRecord & Partial<MapLayerMetadataRow>).properties_count || 0,
                // URLs for frontend
