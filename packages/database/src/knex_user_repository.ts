...
export class KnexUserRepository implements UserRepository {
    ...
    readonly #rolesTable = 'roles'
    readonly #userRolesTable = 'user_roles'
    ...
    async initializeTables(): Promise<void> {
        ...
        if (!(await this.#knex.schema.hasTable(this.#rolesTable))) {
            await this.#knex.schema.createTable(this.#rolesTable, table => {
        ...
        if (!(await this.#knex.schema.hasTable(this.#usersTable))) {
            await this.#knex.schema.createTable(this.#usersTable, table => {
                table.increments('id').primary()
        ...
        if (!(await this.#knex.schema.hasTable(this.#userRolesTable))) {
            await this.#knex.schema.createTable(this.#userRolesTable, table => {
    ...
    async #createUser(authUser: AuthenticatedUser): Promise<UserRecord> {
        const now = new Date()
        ...
        const insertResult = await this.#knex(this.#usersTable).insert(userData).returning('id')
    ...
    async #syncUserRoles(userId: number, newRoles: string[]): Promise<void> {
        await this.#knex.transaction(async trx => {
            ...
            for (const roleName of newRoles) {
                await trx(this.#rolesTable).insert({ name: roleName }).onConflict('name').ignore()
            }
    ...
    async #getUserWithRoles(userId: number): Promise<UserRecord | undefined> {
        const result = (await this.#knex(this.#usersTable)
