...
| File                          | Description                                                                                      |
|-------------------------------|--------------------------------------------------------------------------------------------------|
| `http_integration.spec.ts`    | Boots a full `DigitalTwinEngine` on its Fastify server with Redis queues and registered components. Sends real HTTP requests with `x-user-id` / `x-user-roles` headers (simulating a gateway) and validates status codes, response bodies, and ownership enforcement (401, 403, 404). |
